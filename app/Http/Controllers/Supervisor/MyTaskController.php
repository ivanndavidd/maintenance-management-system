...
class MyTaskController extends Controller
{
    ...
    {
        $query = PmSchedule::where(function ($query) {
            $query->whereHas('scheduleDates.cleaningGroups.sprGroups.tasks', function ($q) {
                $q->where('assigned_user_id', auth()->id());
            })
            ->orWhereHas('scheduleDates.standaloneTasks', function ($q) {
                $q->where('assigned_user_id', auth()->id());
            });
        });

        // Apply filters
        if ($request->filled('scheduled_month')) {
            $query->whereYear('scheduled_month', date('Y', strtotime($request->scheduled_month)))
                  ->whereMonth('scheduled_month', date('m', strtotime($request->scheduled_month)));
        }
        ...
        $schedules = $query->with([
                'scheduleDates.cleaningGroups.sprGroups.tasks' => function ($q) {
                    $q->where('assigned_user_id', auth()->id());
                },
                'scheduleDates.standaloneTasks' => function ($q) {
                    $q->where('assigned_user_id', auth()->id());
                }
            ])
            ->orderBy('scheduled_month', 'desc')
            ->paginate(10);

        return view('supervisor.my-tasks.preventive-maintenance.index', compact('schedules'));
    }
