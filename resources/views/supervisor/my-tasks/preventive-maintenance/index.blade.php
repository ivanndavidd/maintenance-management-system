    ...
    <div class="card mb-4">
        ...
        <div class="collapse show" id="month-{{ $month }}">
            ...
            @php
                ...
                // Expand: today, or has non-approved active tasks on past dates
            ...
            <div class="border-bottom">
                ...
                <div class="px-3 py-2 d-flex justify-content-between align-items-center"
                     ...
                     style="cursor:pointer; background: {{ $isToday ? '#fffbeb' : ($allApproved ? '#f8fffe' : 'transparent') }};">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        @if($isToday)
                            <span class="badge bg-primary" style="font-size:10px;">Today</span>
                        @endif
                        @if($allApproved)
                            <span class="badge bg-success" style="font-size:10px;"><i class="fas fa-check-double me-1"></i>All Done</span>
                        @else
                            @if($pendingCount > 0)
                                <span class="badge bg-warning text-dark" style="font-size:10px;">{{ $pendingCount }} pending</span>
                            @endif
                            @if($doneCount > 0)
                                <span class="badge bg-success" style="font-size:10px;">{{ $doneCount }} done</span>
                            @endif
                        @endif
                    </div>
                ...
                <div class="collapse {{ $expand ? 'show' : '' }}" id="{{ $dayCollapseId }}">
                    <div class="table-responsive">
                        <table class="table table-hover table-compact mb-0" style="font-size:13px;">
                            ...
                            <tbody>
                                ...
                                @endphp
                                <tr class="{{ $reportStatus === 'approved' ? 'table-success' : '' }}">
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        @if($reportStatus)
                                            ...
                                            @if($latestReport->timing_label)
                                                <br>
                                                <span class="badge {{ $latestReport->timing_badge_class }} mt-1" style="font-size:10px;">
