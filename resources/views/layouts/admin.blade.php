...
<head>
    ...
    <style>
        ...
        .sidebar:hover .sidebar-header small {
            opacity: 1;
        }
            ...
            <div class="sidebar" id="sidebar">
                ...
                <nav>
                    ...
                    <a href="{{ route('admin.jobs.index') }}" class="{{ request()->routeIs('admin.jobs.*') ? 'active' : '' }}">
                        <i class="fas fa-wrench"></i><span class="menu-text"> Maintenance Jobs</span>
                    </a>

                    <a href="{{ route('admin.work-reports.index') }}" class="{{ request()->routeIs('admin.work-reports.*') ? 'active' : '' }}">
                        <i class="fas fa-file-alt"></i><span class="menu-text"> Work Reports</span>
                    </a>

                    <hr class="text-white">

                    <a href="{{ route('admin.incident-reports.index') }}" class="{{ request()->routeIs('admin.incident-reports.*') ? 'active' : '' }}">
                        <i class="fas fa-exclamation-triangle"></i><span class="menu-text"> Incident Reports</span>
                        @if(\App\Models\IncidentReport::pending()->count() > 0)
                            <span class="badge bg-danger text-white ms-2">{{ \App\Models\IncidentReport::pending()->count() }}</span>
                        @endif
                    </a>

                    <a href="{{ route('admin.task-requests.index') }}" class="{{ request()->routeIs('admin.task-requests.*') ? 'active' : '' }}">
                        <i class="fas fa-tasks"></i><span class="menu-text"> Task Requests</span>
                        @if(\App\Models\TaskRequest::pending()->count() > 0)
                            <span class="badge bg-warning text-dark ms-2">{{ \App\Models\TaskRequest::pending()->count() }}</span>
                        @endif
                    </a>

                    <a href="{{ route('admin.corrective-maintenance.index') }}" class="{{ request()->routeIs('admin.corrective-maintenance.*') ? 'active' : '' }}">
                        <i class="fas fa-tools"></i><span class="menu-text"> Corrective Maintenance</span>
                        @php
                            $pendingCMR = \App\Models\CorrectiveMaintenanceRequest::whereIn('status', ['pending', 'received'])->count();
                        @endphp
                        @if($pendingCMR > 0)
